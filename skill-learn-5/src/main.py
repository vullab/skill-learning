...
gensettings['condition'] = random.choice(['E+','E-','H+','H-','M',])#['E+','E-','H+','H-','M']
gensettings['subject'] = subjectid
...
def oneTrial(trialdata):
	...
	if trialdata['type']=="test":
		trialdata['ms-prestimblank'] = 1000
		trialdata['ms-precue'] = 0
	elif (trialdata['condition']=='E+' or trialdata['condition']=='E-'):
	...
	drawer.fillScreen(bgcol)
	if trialdata['type']=="train" and (trialdata['condition']=='E+' or trialdata['condition']=='E-' or trialdata['condition']=='M'):
		# display pre-cue
	...
	if -999 in found:
		quit()
	# blank screen
	if trialdata['type']=="test" or trialdata['condition'] != 'M':
...
runExpt(int(gensettings['ntrials']))
