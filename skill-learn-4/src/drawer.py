...
class Drawer(object):
	def __init__(self):
		self.screen = pygame.display.set_mode((1024, 768), pygame.FULLSCREEN)#pygame.FULLSCREEN)#16) # pygame.FULLSCREEN
		self.center = (512,368)
		self.deg2pix = 30
		self.xlim = (0, 1024)
		self.ylim = (0,768)
		...
		for i in range(0,6):
			...
			self.thumbnails.append(pygame.transform.scale(self.images[i], (32, 32)))
	...
	def drawThumb(self, idx, center):
		(w,h) = self.thumbnails[idx].get_size()
		pos = (center[0]-w/2+self.center[0], center[1]-h/2+self.center[1])
		self.screen.blit(self.thumbnails[idx], pos)
