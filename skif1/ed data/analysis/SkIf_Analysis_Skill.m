...
for i =[1:length(sids)]
    ...
    for bk = [1:4]
        bkv = bk+17;
        thisbk = find(data{i,1}(:,1) == bkv);
        
        tbic = find(data{i,1}(thisbk,5)==1);
        tbii = find(data{i,1}(thisbk,5)==0);
        
        bkrtsC{cc,1}(bk,cco(cc)) = mean(data{i,1}(thisbk(tbic),7));
        bkrtsI{cc,1}(bk,cco(cc)) = mean(data{i,1}(thisbk(tbii),7));
        bkacsC{cc,1}(bk,cco(cc)) = mean(data{i,1}(thisbk(tbic),8));
        bkacsI{cc,1}(bk,cco(cc)) = mean(data{i,1}(thisbk(tbii),8));
    end
...
%%
[h p ci stats] = ttest(Ccomp{1,1}, Ccomp{1,2})
[h p ci stats] = ttest(Ccomp{2,1}, Ccomp{2,2})
[h p ci stats] = ttest(rtcomp{1,1}, rtcomp{1,2})
[h p ci stats] = ttest(rtcomp{2,1}, rtcomp{2,2})


[h p ci stats] = ttest2(rtdiff{1}, rtdiff{2})